...
(function(window, document) {
  ...
  window.swal.setDefaults = function(userParams) {
    if (!userParams) {
      throw new Error('userParams الزامی استاد');
    }
    if (typeof userParams !== 'object') {
      throw new Error('userParams باید یک شی باشد');
    }
    ...
    if (params.imageUrl) {
      ...
      if (params.imageSize) {
        ...
        if (!imgWidth || !imgHeight) {
          window.console.error("پارامتر imageSize فقط مقداری با فرمت WIDTHxHEIGHT را قبول می کند. - " + params.imageSize);
        } else {
          _imgWidth  = imgWidth;
