...
namespace Tagro.Admin.Controllers
{
    public class ReportController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                List<Report> reports = ctx.Reports.ToList();
        ...
        [Authorize(Roles = "Admin")]
        public ActionResult List(int? reportStatus)
        {
            using (MyDbContext ctx = new MyDbContext())
            {
                ...
                {
                    reports = ctx.Reports.Where(m => m.Status == reportStatus).ToList();
                }
                else
                {
        ...
        public ActionResult UpdateStatusPublish(int id, byte status)
        {
            using (MyDbContext ctx = new MyDbContext())
            {
                Report report = ctx.Reports.FirstOrDefault(m => m.Id == id);
